...
if(isset($_GET['id'])){
    ...
    $sqlquery = "SELECT * FROM employeedetails where id= $userid";
...
<?php
    ...
    <div class="container mt-3">
        <form action="#" method="POST" id="empform" enctype="multipart/form-data">
            <div class="mb-3 mt-3">
                <label>Name:</label>
                <input type="text" class="form-control" placeholder="Enter Name" name="names" value = "<?php echo $row['names'];  ?>">
            </div>
            <div class="mb-3 mt-3">

                <label>Date of birth:</label>
                <input type="date" id='dob' name='dates' onchange="getAge();" class="form-control" value = "<?php echo $row['dates'];  ?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Enter Mobile no" name="mobno" value = "<?php echo $row['mobno'];  ?>">

                </div>
                <div class="mb-3">
                    <label>Salary:</label>
                    <input type="text" class="form-control" id="salary" placeholder="Enter Salary" name="salary" onchange="changes()" value = "<?php echo $row['salary'];  ?>"><span id="sal"></span>

                </div>
                <div class="mb-3">
                    <label>Profile:</label>
                    <br>
                    <img src="profile/<?php echo $row['profiles'];?>" style="width:10%;height:10%">
                    <br>
                    <br>
           ...
           <div class="mb-3">
                    <label>image:</label>
                            <input type="text" class="form-control" id="salary" placeholder="Enter Salary" name="emp_image" value = "<?php echo $row['profiles'];  ?>"><span id="sal"></span>
                ...
                <button type="submit" class="btn btn-primary" name="update">Update</button> <button type="reset" id="btnreset" class="btn btn-primary" name="add" onClick="document.location.reload(true)">Reset</button>
    ...
    if (move_uploaded_file($_FILES["profiles"]["tmp_name"], "profile/" . $file)) { //move the file to the directory 
        $mysqli= mysqli_query($conn,"UPDATE `employeedetails` SET names='$name',dates='$date',mobno='$mobno',salary='$salary',profiles='$file' WHERE id='$userid'");

    }
    if ($mysqli == true) { //check if the data is inserted 
